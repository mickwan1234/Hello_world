...
import nghia.dtos.CartObj;
import nghia.dtos.RamDTO;
...
public class AddController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(cart == null){
                cart = new CartObj("NghiaNH");
            }
            String product = request.getParameter("ListRam");
            String[] tmp = product.split("-");
            //od-name-price
            RamDTO dto = new RamDTO();
            dto.setId(tmp[0]);
            dto.setName(tmp[1]);
            dto.setPrice(Float.parseFloat(tmp[2]));
            dto.setQuantitiy(1);
